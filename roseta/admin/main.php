...
function roseta_admin_scripts( $hook ) {
	...
	wp_enqueue_style( 'roseta-admin-style', esc_url( get_template_directory_uri() . '/admin/css/admin.css' ), NULL, _CRYOUT_THEME_VERSION );
	wp_enqueue_script( 'roseta-admin-js', esc_url( get_template_directory_uri() . '/admin/js/admin.js' ), array('jquery-ui-dialog'), _CRYOUT_THEME_VERSION );
...
<div class="wrap" id="main-page"><!-- Admin wrap page -->
	...
	if ( isset( $_POST['cryout_reset_defaults'] ) ) {
		...
		<div id="admin_header">
			...
			<span class="version">
				<?php echo wp_kses_post( apply_filters( 'cryout_admin_version', sprintf( __( 'Roseta Theme v%1$s by %2$s', 'roseta' ),
