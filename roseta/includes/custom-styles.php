...
function roseta_body_classes( $classes ) {
	$options = cryout_get_option( array(
		...
		'theme_normalizetags', 'theme_articleanimation', 'theme_headertitles_archives', 'theme_lazyimages'
	) );
	...
	if ( $options['theme_menuposition'] && !empty( $header_image ) ) $classes[] = 'roseta-over-menu';
	...
	if ( $options['theme_comlabels'] == 3 ) $classes[] = 'roseta-comment-optlabels';
	...
	if ( $options['theme_lazyimages'] == 2 ) $classes[] = 'roseta-lazy-noanimation';
...
#header-page-title .entry-title {
	font-family: <?php cryout_font_select( $theme_fht_title, $theme_fht_titlegoogle, true ) ?>;
	...
	<?php echo ( ! empty( $theme_fht_titlevariant ) ) ? 'text-transform: ' . esc_attr( $theme_fht_titlevariant ) : ''; ?>;
}

#header-page-title .entry-meta > span {
	font-family: <?php cryout_font_select( $theme_fht_meta, $theme_fht_metagoogle, true ) ?>;
	font-size: <?php echo esc_html( $theme_fht_metasize ) ?>em; font-weight: <?php echo esc_html( $theme_fht_metaweight ) ?>;
	<?php echo ( ! empty( $theme_fht_metavariant ) ) ? 'text-transform: ' . esc_attr( $theme_fht_metavariant ) : ''; ?>;
}
...
#access .children li:not(:last-child) a > span {
	border-bottom: 1px solid <?php echo esc_html( cryout_hexdiff( $theme_submenubackground, 17 ) ); ?>;
}
...
.seriousslider-theme .seriousslider-caption::after {
	background-color: <?php echo esc_html( $theme_overlaybackground ) ;?>;
	opacity: <?php echo esc_html( $theme_overlayopacity/100 ); ?>;
}

#header-page-title #header-page-title-inside,
#header-page-title .entry-meta span, #header-page-title .entry-meta a,
#header-page-title .entry-meta time, #header-page-title .entry-meta .icon-metas::before,
...
.author-description .page-title span {
	color:  <?php echo esc_html( $theme_accent2 ) ?>;
	border-bottom-color: <?php echo esc_html( $theme_accent1 ) ?>;
}
...
.commentlist .comment-body::after {
	border-right-color: <?php echo esc_html( cryout_hexdiff( $theme_contentbackground, 0 ) ) ?>;
}

.commentlist .comment-body::before {
	border-right-color: <?php echo esc_html( cryout_hexdiff( $theme_contentbackground, 23 ) ) ?>;
}

article .author-info, .single .author-info {
	border-color: <?php echo esc_html( cryout_hexdiff( $theme_contentbackground, 17 ) ) ?>;
}
...
.comment-meta a {
	color: <?php echo esc_html( cryout_hexdiff( $theme_sitetext, -99) ) ?>;
}
...
@media (min-width: 1152px) {
	<?php if ( esc_html( $theme_menuposition ) ) { ?>
	body:not(.roseta-landing-page) #content {
		margin-top: <?php echo intval( $theme_menuheight ) + 100 ?>px;
	}
	body.roseta-over-menu #site-header-main {
		background-color: <?php echo esc_html( $theme_menubackground ); ?>; 
	}
	<?php } ?>
...
.lp-staticslider {
	max-height: calc(100vh - <?php echo intval( $theme_menuheight + 100 ) ?>px);
}
...
.lp-block {
	background: <?php echo esc_html( $theme_contentbackground ); ?>;
...
.lp-boxes-static .lp-box-image .box-overlay {
	background-color: <?php echo esc_html( cryout_hexdiff( $theme_accent1, -20 ) ); ?>;
}
.lp-box-title {
	color: <?php echo esc_html( $theme_accent2 ) ?>;
}
...
.lp-boxes-2 .lp-box .lp-box-image {
	height: <?php echo intval ( (int) $theme_lpboxheight2 ); ?>px;
}
