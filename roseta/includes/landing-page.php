...
* slider builder
*/
if ( ! function_exists('roseta_lpslider' ) ):
function roseta_lpslider() {
...
* slider output
*/
if ( ! function_exists( 'roseta_lpslider_output' ) ):
function roseta_lpslider_output( $data ){
extract($data);
 ...
 * blocks builder
 */
if ( ! function_exists( 'roseta_lpblocks' ) ):
function roseta_lpblocks( $sid = 1 ) {
	$maintitle = cryout_get_option( 'theme_lpblockmaintitle'.$sid );
	$maindesc = cryout_get_option( 'theme_lpblockmaindesc'.$sid );
	$pageids = cryout_get_option( apply_filters('roseta_blocks_ids', array( 'theme_lpblockone'.$sid, 'theme_lpblocktwo'.$sid, 'theme_lpblockthree'.$sid, 'theme_lpblockfour'.$sid), $sid ) );
	$icon = cryout_get_option( apply_filters('roseta_blocks_icons', array( 'theme_lpblockoneicon'.$sid, 'theme_lpblocktwoicon'.$sid, 'theme_lpblockthreeicon'.$sid, 'theme_lpblockfouricon'.$sid ), $sid ) );
	...
	?>
	<section id="lp-blocks<?php echo absint( $sid ) ?>" class="lp-blocks lp-blocks<?php echo absint( $sid ) ?> lp-blocks-rows-<?php echo esc_attr( apply_filters('roseta_blocks_perrow', $pagecount, $sid) ) ?>">
			...
			<?php foreach ( $pageids as $key => $pageid ) {
				...
				if ( intval( $pageid ) > 0 ) {
					...
					switch ( $blockscontent ) {
						...
						case '1': default: if (has_excerpt( $pageid )) $text = get_the_excerpt( $pageid ); else $text = roseta_custom_excerpt( apply_filters( 'the_content', get_post_field( 'post_content', $pageid ) ) ); break;
					};

					$iconid = preg_replace('/(\d)$/','icon$1', $key);

					$data[$count] = array(
						'title' => apply_filters('roseta_block_title', get_the_title( $pageid ), $pageid ),
						'text'	=> $text,
						'icon'	=> ( ( $icon[$iconid] != 'no-icon' ) ? $icon[$iconid] : '' ),
						'link'	=> apply_filters( 'roseta_block_url', get_permalink( $pageid ), $pageid ),
						'target' => apply_filters( 'roseta_block_target', '', $pageid ),
						'click'	=> $blocksclick,
						'id' 	=> $count,
						'readmore' => $blocksreadmore,
					);
					roseta_lpblock_output( $data[$count] );
					$count++;
				}
			} ?>
		</div>
 ...
 * blocks output
 */
if ( ! function_exists( 'roseta_lpblock_output' ) ):
function roseta_lpblock_output( $data ) { ?>
	<?php extract($data) ?>
 ...
 * boxes builder
 */
if ( ! function_exists( 'roseta_lpboxes' ) ):
function roseta_lpboxes( $sid = 1 ) {
		...
		<section id="lp-boxes-<?php echo absint( $sid ) ?>" class="lp-boxes lp-boxes-<?php echo absint( $sid ) ?> <?php  echo esc_attr( $animated_class ) ?> lp-boxes-rows-<?php echo absint( $options['theme_lpboxrow' . $sid] ); ?>">
    		...
    		<?php while ( $custom_query->have_posts() ) :
					...
					if ( cryout_has_manual_excerpt( $custom_query->post ) ) {
						...
						$excerpt = roseta_custom_excerpt( get_the_excerpt(), $options['theme_lpboxlength' . $sid] );
					} else {
						$excerpt = roseta_custom_excerpt( get_the_content(), $options['theme_lpboxlength' . $sid] );
					};
		            $box = array();
		            $box['colno'] = $box_counter++;
		            $box['counter'] = $options['theme_lpboxcount' . $sid];
		            $box['title'] = apply_filters( 'roseta_box_title', get_the_title(), get_the_ID() );
		            $box['content'] = $excerpt;
		            $box['image'] = wp_get_attachment_image( get_post_thumbnail_id( get_the_ID() ), 'roseta-lpbox-' . $sid );
					$box['link'] = apply_filters( 'roseta_box_url', get_permalink(), get_the_ID() );
					$box['readmore'] = apply_filters( 'roseta_box_readmore', $options['theme_lpboxreadmore' . $sid], get_the_ID() );
					$box['target'] = apply_filters( 'roseta_box_target', '', get_the_ID() );

					$box['image'] = apply_filters( 'roseta_preview_img_src', $box['image'] );

            		roseta_lpbox_output( $box );
        		endwhile; ?>
			</div>
		</section><!-- .lp-boxes -->
<?php endif;
 ...
 * boxes output
 */
if ( ! function_exists( 'roseta_lpbox_output' ) ):
function roseta_lpbox_output( $data ) {
 ...
 * text area output
 */
if ( ! function_exists( 'roseta_lptext_output' ) ):
function roseta_lptext_output( $data ){ ?>
...
<?php
} // roseta_lptext_output()
endif;

/**
 * page or posts output, also used when landing page is disabled
 */
if ( ! function_exists( 'roseta_lpindex' ) ):
function roseta_lpindex() {
